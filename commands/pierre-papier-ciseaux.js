  ...
  execute: async ({ interaction, channel, options, user, client }) => {
    ...
    if (user.id == player2 || (userObj.bot && player2 !== client.user.id)) {
      await interaction.reply({
        content: "Impossible de jouer contre soi-même ou contre un bot...",
        ephemeral: true,
      });
      return;
    }
    const reponse =
      "<@" +
      player2 +
      ">, tu as été défié au **shifumi** par <@" +
      player1 +
      "> !\n\nFais ton choix :";
    ...
    collector.on("collect", async (i) => {
      ...
      if (
        (i.user.id === player1 || i.user.id === player2) &&
        i.customId in signs
      ) {
        if (i.user.id === player1 && !player1_choice) {
          player1_choice = i.customId;
          await i.reply({
            content: 'Tu as sélectionné "' + i.customId + '".',
            ephemeral: true,
          });
        ...
        if (player1_choice && player2_choice) {
          ...
          await interaction.editReply({
            content:
              ...
              player1 +
              "> !\n\n" +
              signs[player1_choice].icon +
              " *<@" +
              player1 +
              ">*  **VS**  " +
              signs[player2_choice].icon +
              " *<@" +
              player2 +
              ">*\n" +
              sentence,
      ...
      if (!player1_choice || !player2_choice) {
        ...
        if (player1_choice) {
          ...
          response =
            ...
            "> et <@" +
            player1 +
            ">, vous avez mis trop de temps à réagir.";
        }
